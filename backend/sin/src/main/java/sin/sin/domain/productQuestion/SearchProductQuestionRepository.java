...
public class SearchProductQuestionRepository {
    ...
    public Page<ProductQnaResponse> findProductQnaByProductCode(String productCode, Pageable pageable) {
        JPAQuery<ProductQnaResponse> query = queryFactory.select(new QProductQnaResponse(
                        new QProductQuestionResponse(productQuestion.id, productQuestion.title, productQuestion.content, member.name, productQuestion.secret, productQuestion.createdDate),
                        new QProductQuestionReplyResponse(productQuestionReply.id, productQuestionReply.Content, productQuestionReply.createdDate)))
