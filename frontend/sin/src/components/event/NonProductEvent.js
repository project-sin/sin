...
const NonProductEvent = () => {
  ...
  const color = (parentid) => {
    ...
    document.getElementById(`${parenton}`).childNodes.forEach(e=>{e.style.color = '#5f0080'})
    ...
    document.getElementById(`${parentid}`).childNodes.forEach(e=>{e.style.color = '#fff'})
          ...
          <NonProducteventwrap>
            ...
            <Right>
              <Div1>
                ...
                <Subtitle>고객님께 받은 사랑을 혜택으로 돌려드려요. 매월 실적에 따라 달라지는 실용적이고 다채로운 혜택을 확인해보세요.</Subtitle>
              </Div1>
              <Div2>
                <Thepurple id='parentthepurple' onClick={()=>{handler('thepurple');color('parentthepurple')}}><Name>더퍼플</Name><Rate>7%</Rate></Thepurple>
                <Purple id='parentpurple' onClick={()=>{handler('purple');color('parentpurple')}}><Name>퍼플</Name><Rate>7%</Rate></Purple>
                <Lavender id='parentlavender' onClick={()=>{handler('lavender');color('parentlavender')}}><Name>라벤더</Name><Rate>5%</Rate></Lavender>
                <White id='parentwhite' onClick={()=>{handler('white');color('parentwhite')}}><Name>화이트</Name><Rate>3%</Rate></White>
              ...
              <Div4>
                ...
                <Div4p>총 적립 금액은 12개월간 동일 등급을 유지할 경우 받게 되는 적립 금액의 총합계입니다.</Div4p>
